...
from algo.sde.ornstein_uhlenbeck_parameters import ModelParamsOU


def model_params_ou(x: np.ndarray, dt: float) -> ModelParamsOU:
    n = x.shape[0]
    ...
    return ModelParamsOU(theta=theta, mu=mu, sigma_sq=sigma_sq)


def log_likelihood_ou(theta: float, mu: float, sigma_sq: float, x: np.ndarray, dt: float) -> float:
    n = x.shape[0]
...
def loglikelihood_candidates_ou(
        asset1: np.ndarray, asset2: np.ndarray, dt: float, alpha: float, B_candidates: List[float]
) -> Tuple[List[float], List[ModelParamsOU]]:
    """
    ...
    for B in B_candidates:
        ...
        ou_params = model_params_ou(x, dt)
        ou_params_candidates.append(ou_params)

        # Log-likelihood of observing data given the model parameters.
...
# TODO: deprecate
# def hedging_parameters_ou(log_likelihoods, ou_params_candidates, B_candidates, S2_0, A, alpha):
#     """
#
#     Args:
#         log_likelihoods:
