...
class OUPairsTradingStrategy(PairsTradingStrategy):
    ...
    def __init__(
        ...
        self.roll_long = False
        self.roll_short = False
    ...
    def next(self):
        ...
        if self.step >= self.num_test \
                ...
                and not self.is_order_pending:
                # and self.order_buy is None \
                # and self.order_sell is None \
                # and self.order_buy_roll is None \
                # and self.order_sell_roll is None:
            self.train()
        ...
        if z_score <= -self.z_entry and not self.in_market and not self.is_rolling:
        ...
        elif z_score >= self.z_entry and not self.in_market and not self.is_rolling:
        ...
        # elif z_score >= -self.z_exit and self.in_market:
    ...
    def notify_order(self, order):
        if order.status in [bt.Order.Submitted, bt.Order.Accepted]:
            ...
            if order.isbuy():
                ...
                elif self.order_buy_roll is not None and order.ref == self.order_buy_roll.ref:
                ...
                elif self.order_sell_roll is not None and order.ref == self.order_sell_roll.ref:
    ...
    def roll(self):
        if not self.in_market or self.is_rolling:
            return
        ...
        if self.data0.roll_date == 1:
            ...
            if size0 > 0.0:
                self.order_buy_roll = self.buy(data=self.data0, size=size0, exectype=bt.Order.Market)
                self.order_sell_roll = self.sell(data=self.data0, size=size0, exectype=bt.Order.Market)
                self.roll_long = True

            elif size0 < 0.0:
                self.order_sell_roll = self.sell(data=self.data0, size=np.abs(size0), exectype=bt.Order.Market)
                self.order_buy_roll = self.buy(data=self.data0, size=np.abs(size0), exectype=bt.Order.Market)
                self.roll_short = True
        ...
        if self.data1.roll_date == 1:
            ...
            # Do nothing if size is 0.
            if size1 > 0.0:
                self.order_buy_roll = self.buy(data=self.data1, size=size1, exectype=bt.Order.Market)
                self.order_sell_roll = self.sell(data=self.data1, size=size1, exectype=bt.Order.Market)
                self.roll_short = True

            elif size1 < 0.0:
                self.order_buy_roll = self.buy(data=self.data1, size=np.abs(size1), exectype=bt.Order.Market)
                self.order_sell_roll = self.sell(data=self.data1, size=np.abs(size1), exectype=bt.Order.Market)
                self.roll_long = True
